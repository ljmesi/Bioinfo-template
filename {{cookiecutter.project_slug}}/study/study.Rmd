...
```{bash}
ENV_DEFINITION_FILE="environment_definition.txt"
if [ ! -f "$ENV_DEFINITION_FILE" ]; then
    conda list --explicit > "$ENV_DEFINITION_FILE"
    echo "Writing environment definition file: $ENV_DEFINITION_FILE" 
else
    echo "Environment definition file: \"$ENV_DEFINITION_FILE\" already exists, skipping the writing..."
fi
```
...
```{bash}
conda list --explicit > env_info.txt
conda env export --name base > env_info.yml
```
